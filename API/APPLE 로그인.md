...
- Client가 인증을 통해 토큰을 가져오면 Backend에서 토큰을 가지고 고객 정보를 가져와 회원 가입 처리를 한다.

- Apple에서는 JSON Web Tokens를 주는데 payload 파트에서 필요한 고객정보를 얻을 수 있다.

- 따라서 추가로 정보 요청을 할 필요 없이 JWT가 변형없이 정상적으로 왔는지 검증만 하면 된다.
