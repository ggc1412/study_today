...
2. 로그아웃 & 연결 끊기
   ...
   - 로그아웃 - 로그인을 통해 발급 받은 사용자 토큰을 만료시켜 처리
...
4. 사용자 인증
   ...
   - 카카오톡으로 로그인은 REST API를 활용할 경우 불가능. 이때는 계정 정보를 웹페이지에 입력해야함
...
- Android
  ...
  Intent URI를 이용
  Javscript SDK가 카카오톡 실행을 위한 `Intent URI`를 만들어서 호출합니다. 웹뷰에서는 [WebViewClient#shouldOverrideUrlLoading](https://developer.android.com/reference/android/webkit/WebViewClient#shouldOverrideUrlLoading(android.webkit.WebView, android.webkit.WebResourceRequest)) 메소드를 오버라이딩(Override)하여 `intent`를 파싱하고 해당 `Activity`를 실행해야 합니다.
